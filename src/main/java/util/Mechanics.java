...
public class Mechanics {
	...
	public Mechanics(Board sudokuBoard, ArrayList<Highlight> high){
	...
	public ArrayList<Integer> checkRowAt(int x, int y, int pos){
		if(sudokuBoard.getDigit(pos).getType() == VALUE_TYPE.EMPTY || sudokuBoard.getDigit(pos).getType() == VALUE_TYPE.GUESS){
	        ...
	        for(int i=0; i<sudokuBoard.getNumberCount(); i++){	
	        	...
	        	if(i != x && current.size() == 1 && current.getType() != VALUE_TYPE.GUESS){
	        		check.remove((Integer)current.get(0));
	        	}
	        }
			sudokuBoard.getDigit(pos).setType(VALUE_TYPE.GUESS);
	        return check;
		}
		else
	...
	public ArrayList<Integer> checkColumnAt(int x, int y, int pos){
		if(sudokuBoard.getDigit(pos).getType() == VALUE_TYPE.EMPTY || sudokuBoard.getDigit(pos).getType() == VALUE_TYPE.GUESS){
	        ArrayList<Integer> check = new ArrayList<>();
	        for(int i=1; i<sudokuBoard.getNumberCount()+1; i++) check.add((Integer)i);
	        
	        for(int i=x; i<sudokuBoard.getBoxCount(); i+=sudokuBoard.getNumberCount()){
	        	Digit current = sudokuBoard.getDigit(i);
	        	if(i != pos && current.size() == 1 && current.getType() != VALUE_TYPE.GUESS){
	        		check.remove((Integer)current.get(0));
	        	}
	        }
			sudokuBoard.getDigit(pos).setType(VALUE_TYPE.GUESS);
	...
	public ArrayList<Integer> checkBoxAt(int x, int y, int pos) {
		if(sudokuBoard.getDigit(pos).getType() == VALUE_TYPE.EMPTY || sudokuBoard.getDigit(pos).getType() == VALUE_TYPE.GUESS) {
	        ...
	        for(int i=0; i<sudokuBoard.getNumberCount(); i++) {
	        	...
	        	if(fromPos != pos && current.size() == 1 && current.getType() != VALUE_TYPE.GUESS){
	        		check.remove((Integer)current.get(0));
	        	}
	        	if((1+i)%sudokuBoard.getBoxWidth() == 0)
	        		fromPos = fromPos-(sudokuBoard.getBoxWidth()-1)+sudokuBoard.getNumberCount();
	        	else
	        		fromPos++;
	        }
			sudokuBoard.getDigit(pos).setType(VALUE_TYPE.GUESS);
	...
	public boolean solvedAt(int pos){
		...
		if(current.size() == 1 && current.getType() == VALUE_TYPE.GUESS){
			current.setType(VALUE_TYPE.NUMBER);
	...
	public int scanRowAt(int x, int y, int pos){
		...
		for(int i=0; i<sudokuBoard.getNumberCount(); i++){
			...
			if(i != x && current.getType() == VALUE_TYPE.GUESS){
	...
	public int scanColumnAt(int x, int y, int pos){
		...
		for(int i=0; i<sudokuBoard.getNumberCount(); i++){
			...
			if(i != y && current.getType() == VALUE_TYPE.GUESS){
				for(int j = 0; j<current.size(); j++){
					currentList.remove((Integer)current.get(j));
				}
			}
	...
	public int scanBoxAt(int x, int y, int pos){
		...
		for(int i=0; i<sudokuBoard.getNumberCount(); i++){
			...
			if(pos != fromPos && current.getType() == VALUE_TYPE.GUESS){
				for(int j = 0; j<current.size(); j++){
					currentList.remove((Integer)current.get(j));
				}
			}
	...
	public boolean scanAll(){
		boolean found = false;
		for(int i=0; i<sudokuBoard.getNumberCount() && !found; i++){
			for(int j=0; j<sudokuBoard.getNumberCount() && !found; j++){
				found = scanAt(j,i);
			}
		}
		if(found)
	...
	public boolean solutionCheck(){
		...
		for(int i=0; i<sudokuBoard.getBoxCount(); i++){
			Digit current = sudokuBoard.getDigit(i);
			if(current.getType() == VALUE_TYPE.NUMBER){
			...
			else if (current.getType() != VALUE_TYPE.CLUE){
				finished = false;
			}
		}
		if(!finished)
	...
	public void solutionShow(){
		for(int i = 0; i<sudokuBoard.getBoxCount(); i++){
			if(sudokuBoard.getDigit(i).getType() != VALUE_TYPE.CLUE){
				sudokuBoard.getDigit(i).add(sudokuBoard.getAnswer(i), VALUE_TYPE.NUMBER);
			}
		}
	}
